            ...
            {messages.map((msg) => {
                ...
                if (isMe) {
                    ...
                    if (subLevel === 'ULTRA') {
                        ...
                        glowClass = 'shadow-[0_0_10px_rgba(234,179,8,0.3)]'; // Gold Glow
                        animationClass = animationClass || 'animate-in zoom-in-95 duration-300';
                         ...
                         <div className={`max-w-[85%] rounded-2xl p-3 shadow-sm relative group transition-all duration-300 ${bubbleClass} ${glowClass}`}>
                             <div className="flex justify-between items-start gap-2 mb-1">
                                 <span className={`text-[10px] font-bold uppercase tracking-wider flex items-center gap-1 ${isMe ? 'text-blue-200' : isAdminMsg ? 'text-yellow-400' : 'text-slate-500'}`}>
                                     {isAdminMsg && <Crown size={12} className="text-yellow-400 animate-bounce" />}
                                     {/* @ts-ignore */}
                                     {!isAdminMsg && !isMe && subLevel === 'ULTRA' && <Zap size={10} className="text-yellow-500 fill-yellow-500" />}
